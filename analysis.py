...
# This function must be OUTSIDE the class and properly indented
def calculate_mmi(vix, nifty_df, fii_flow):
    """
    Optimized MMI with proper indentation and safety gates.
    """
    # [span_5](start_span)Safety gate: Ensure DataFrame has enough data[span_5](end_span)
    if nifty_df is None or nifty_df.empty or len(nifty_df) < 95:
        return 50.0

    try:
        # 1. [span_6](start_span)Trend Momentum (90-day SMA)[span_6](end_span)
        ...
        # 2. [span_7](start_span)Volatility (VIX Mean Reversion)[span_7](end_span)
        vix_val = vix.item() if hasattr(vix, 'item') else float(vix)
        vix_score = max(min(100 - ((vix_val - 12) * 6.5), 100), 0)

        # 3. [span_8](start_span)Market Demand (RSI) with Safety Gate[span_8](end_span)
        import pandas_ta as ta
        ...
        # 4. [span_9](start_span)FII Influence[span_9](end_span)
        fii_score = 100 if float(fii_flow) > 0 else 0

        # [span_10](start_span)Final Weighted Calculation[span_10](end_span)
        mmi = (vix_score * 0.3) + (momentum_score * 0.3) + (demand_score * 0.2) + (fii_score * 0.2)
        return round(max(min(mmi, 100), 0), 2)
        
    except Exception as e:
        # [span_11](start_span)Catch-all to prevent app crashes if logic fails[span_11](end_span)
        print(f"MMI Error: {e}")
        return 50.0
